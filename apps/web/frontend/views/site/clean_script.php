...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Load Areas
    try {
        const res = await fetch('https://isurf.digdaya.net/isurf/v1/api/areas/');
        if (res.ok) {
    ...
    areaSelect.addEventListener('change', async (e) => {
        ...
        try {
            ...
            const [areaRes, sensorsRes, actuatorsRes, conditionsRes, schedulesRes] = await Promise.all([
                fetch(`https://isurf.digdaya.net/isurf/v1/api/areas/${areaId}`),
                fetch(`https://isurf.digdaya.net/isurf/v1/api/sensors/`),
                fetch(`https://isurf.digdaya.net/isurf/v1/api/actuators/`),
                fetch(`https://isurf.digdaya.net/isurf/v1/api/areas/${areaId}/conditions`),
                fetch(`https://isurf.digdaya.net/isurf/v1/api/areas/${areaId}/schedules`)
            ]);
...
window.submitNewAreaCondition = async function() {
    try {
        await fetch(`https://isurf.digdaya.net/isurf/v1/api/areas/${currentManageAreaId}/conditions`, {
            method: 'POST',
...
window.deleteAreaCondition = async function(ruleId) {
    ...
    try {
        await fetch(`https://isurf.digdaya.net/isurf/v1/api/areas/${currentManageAreaId}/conditions/${ruleId}`, {
...
window.submitNewAreaSchedule = async function() {
    try {
        await fetch(`https://isurf.digdaya.net/isurf/v1/api/areas/${currentManageAreaId}/schedules`, {
            method: 'POST',
            headers: {'Content-Type': 'application/json', 'Authorization': 'Bearer ' + ISURF_API.apiKey},
            body: JSON.stringify({
...
window.deleteAreaSchedule = async function(ruleId) {
    ...
    try {
        await fetch(`https://isurf.digdaya.net/isurf/v1/api/areas/${currentManageAreaId}/schedules/${ruleId}`, {
            method: 'DELETE', headers: {'Authorization': 'Bearer ' + ISURF_API.apiKey}
        });
        window.refreshCurrentArea();
    } catch(e) { alert('Error: ' + e); }
...
window.submitBulkThreshold = async function() {
    try {
        await fetch(`https://isurf.digdaya.net/isurf/v1/api/areas/${currentManageAreaId}/sensors/thresholds`, {
            method: 'PUT',
            headers: {'Content-Type': 'application/json', 'Authorization': 'Bearer ' + ISURF_API.apiKey},
            body: JSON.stringify({
