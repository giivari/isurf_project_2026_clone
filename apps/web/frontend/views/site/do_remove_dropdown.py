...
filepath = r'c:\Users\givar\KULIAH\capstone\ilkom-isurf-project\apps\web\frontend\views\site\area-details.php'

with open(filepath, 'r', encoding='utf-8') as f:
    orig = f.read()

# 1. Remove HTML Dropdown block
html_to_remove = '''    <!-- Select Area -->
    <div class="glass-panel p-6 mb-6" style="border-radius: 16px;">
        <label for="areaSelect" class="block text-sm font-medium text-gray-700 mb-2">Pilih Area Operasional</label>
        <select id="areaSelect" class="ds-input w-full md:w-1/3">
            <option value="">Memuat data area...</option>
        </select>
    </div>'''
orig = orig.replace(html_to_remove, '')

# 2. Extract the script part and replace it
...
orig = orig[:script_start] + new_script
